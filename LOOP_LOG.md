...
1. **배송 경로 시각화**: 마스코트 캐릭터가 경로(물류센터→허브→터미널→배송지점→집)를 따라 이동하는 "배송 여정 맵". 에셋 규격은 ASSETS.md 참조. `assets/route/` 의 char-*.gif(6) / node-*.png(5)를 <img>로 슬롯 배치하고, 파일 없으면 emoji 플레이스홀더로 폴백. data-status별 캐릭터/위치 매핑은 ASSETS.md 표 그대로. (사용자가 이미지/gif 제작 예정)
